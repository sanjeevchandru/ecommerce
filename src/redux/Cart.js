...
export const Cart=()=>{
    ...
    useEffect(() => {
        let x = a.arr.filter((e) => { return e.isadd })
        setaob(x)
    }, [aob, a.arr])
    const handleFav = (i) => {
        let x = a.arr.map((e) => { return e.id === i ? { ...e, isfav: !e.isfav } : e })
        b(update(x))
    }
    ...
    const increcart=(id)=>{
        ...
        if(y===true){
            ...
            b(update(x))
            alert("don't  select the same item ,more than 10 times")
        }
        else{
            b(update(x))
        }
    ...
    const setprice=(p,index,key)=>{
        let x=a.arr.map((e,i)=>{
            return key===e.id? {...e,originalKg:p.kg,originalPrice:p.price,isprice:false,weight:e.weight.map((ei,ind)=>{return index===ind?{...ei,isActive:true}:{...ei,isActive:false} })}:e})
            b(update(x))
        setaob(x)
    }
    const logOut=()=>{
                            ...
                            <div key={index} className="col">
                                <div className="card">
                                                ...
                                                return(
                                                    <div key={i} className="kg-col">
                                                        <div className="kg-card">
                                                            <div className="kg-p" onClick={()=>setprice(v,i,e.id)}>
                                                            <h3>{v.kg}</h3>
                                    ...
                                    <div className="btn-row">
                                        ...
                                        <div onClick={() => handleFav(e.id)} className="fav-btn" title="Remove to favourite">
