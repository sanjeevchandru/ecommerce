...
export const Detail = () => {
    ...
    const b=useDispatch()
    const [param] = useSearchParams("i");
    const [obj, setobj] = useState([]);
    const nav=useNavigate();
    useEffect(() => {
        let x = a.arr.filter((e) => { return e.id === Number(param.get("i")) ? e : "" })
        setobj(x)
        console.log(x);
    }, [obj, param, a])
    const addcart = (id) => {
    ...
    const setprice = (p, index) => {
        //arr of object index=>key
        //index =>weight index
        //p => weight object
        console.log(p)
        let x = a.arr.map((e) => {
            return e.id === Number(param.get("i")) ? { ...e, originalKg: p.kg, offer:p.offer,originalPrice: p.price, weight: e.weight.map((ei, ind) => { return index === ind ? { ...ei, isActive: true } : { ...ei, isActive: false } }) } : e
        })
        b(update(x))
        // setobj(x)
    }
                ...
                <div className="container">
                    <div className="nav-row">
                        ...
                        <div className="nav-col">
                            ...
                            <div>
                                <p style={{fontSize:"20px",cursor:"pointer"}} onClick={()=>logOut()}><BiSolidLogOutCircle/><span>Out</span></p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="det-row">
                    {
                        obj.map((obj, index) => {
                            return (
                                <div>
                                    <div key={index} className="det-row">
                                        ...
                                        <div className="det-col">
                                            <div className="det-card">
                                                <span>{obj.brand}</span>
                                                <h4>{obj.name} {obj.originalKg}</h4>
                                                <h4>Price :{obj.originalPrice} <span> ( {obj.originalPrice} / kg)</span></h4>
                                                <p>You Save : {obj.offer}</p>
                                                <div className="btn-row">
                                                    {obj.isadd ?
                                                        <div className="addtrue">
                                                            <button onClick={() => decrecart(obj.id)}>-</button>
                                                            <p>{obj.count}</p>
                                                            <button onClick={() => increcart(obj.id)}>+</button>
                                                        </div> :
                                                        <div className="btn1"  onClick={() => addcart(obj.id)}>
                                                    ...
                                                    {obj.weight.map((v, i) => {
                                                        return (
                                                            <div key={i} className="extra-col">
                                                                 <div className="extra-card" style={v.isActive?{borderColor:"rgb(118, 185, 0)",boxShadow:"0 0 5px rgb(118, 185, 0)"}:{borderColor:"#ccc"}}  onClick={() => setprice(v, i)}>
                                                                <div>
                                                                        <p>{v.kg}</p>
                                                                    </div>
                                                                    <div>
                                    ...
                                    <h3 style={{ textAlign: "left", padding: "10px 0" }}>Fresho {obj.name} </h3>
